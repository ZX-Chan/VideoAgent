...
from core.gen_outline_layout import filter_image_table
...
# 要点生成agent
def generate_bullet_points_with_agent(content, agent_config=None, prompt_path=None, section_title=None):
    """
    调用要点生成agent，输入原始内容，返回精简的要点列表。
    prompt_path: prompt yaml路径
    agent_config: 可选，默认4o
    """
    if agent_config is None:
        ...
        agent_config = get_agent_config('4o')
    if prompt_path is None:
        prompt_path = 'utils/prompt_templates/bullet_point_generator_improved.yaml'
    with open(prompt_path, 'r') as f:
        prompt_config = yaml.safe_load(f)
    jinja_env = Environment(undefined=StrictUndefined)
    template = jinja_env.from_string(prompt_config["template"])
    jinja_args = {"content": content}
    if section_title is not None:
        jinja_args["section_title"] = section_title
    prompt = template.render(**jinja_args)
    model = ModelFactory.create(
        model_platform=agent_config['model_platform'],
        model_type=agent_config['model_type'],
    ...
    response = agent.step(prompt)
    input_token, output_token = account_token(response)
    bullet_points_json = get_json_from_response(response.msgs[0].content)
    
    return bullet_points_json, input_token, output_token
